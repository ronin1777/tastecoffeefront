  ...
  return (
    <>
      {isAuthenticated ? (
        <div className="md:absolute-login-link md:fixed md:fix-login-link hidden md:block md:left-5 md:top-14 md:lg:left-[70px] text-orange-200 z-50 tracking-tight">
          <div className="relative group">
            <span className="group-hover:text-orange-300  top-10 transition-colors mt-1.5 lg:truncate">
              {user?.name?.split(" ")[0]}
            </span>
            <div className="absolute flex left-4 flex-col p-5 top-10 space-y-3 w-36 invisible group-hover:opacity-100 group-hover:visible transition-all bg-white dark:bg-zinc-700 text-zinc-700 dark:text-white text-base rounded-2xl border-t-[3px] border-orange-300 shadow-normal child:transition-colors child-hover:text-white dark:child-hover:text-orange-300 tracking-normal">
              <Link
                ...
                className="flex items-center gap-x-2 hover:text-white  rounded hover:bg-orange-400 transition-colors text-base"
              >
                <FaUser />
                <span>پروفایل</span>
              </Link>
        ...
        >
          <div className="flex">
            ...
            <span className="md:hidden text-orange-300 dark:tex lg:block lg:max-w-48 lg:truncate">
              ورود | ثبت نام
            </span>
          </div>
        </button>
      ...
      {isModalOpen && (
        ...
        >
          <div className="bg-white flex gap-12 flex-col justify-start dark:bg-zinc-700 rounded-lg shadow-lg w-[100%] h-[290px] sm:w-[550px] max-w-lg mx-auto my-auto relative top-1/2 transform -translate-y-1/2">
