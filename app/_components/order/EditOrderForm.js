...
export default function EditOrderForm({ initialData, onUpdate, accessToken }) {
    ...
    const onSubmit = async (formData) => {
        const response = await fetch(`http://127.0.0.1:8000/api/orders/orders-update/${formData.id}/`, {
            method: 'PATCH',
            headers: {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className='my-10'>
            ...
            <div className="mb-6">
                <label className="block text-zinc-800 dark:text-white text-sm font-semibold mb-2"
