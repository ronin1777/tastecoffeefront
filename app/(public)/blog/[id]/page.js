...
const BlogPostDetail = async ({ params }) => {
    ...
    const res = await fetch(`http://127.0.0.1:8000/api/blog/blog/${id}/`);
...
return (
        <div className="container my-40 mx-auto p-6">
            <h1 className="text-3xl font-bold text-zinc-700 dark:text-white text-center mb-4">{blogPost.title}</h1>
            ...
            <div className="space-y-4">
                ...
                <div
                    ...
                    dangerouslySetInnerHTML={{ __html: blogPost.paragraphs.map(p => p.content).join('') }}
                />
            </div>
        </div>
    );
